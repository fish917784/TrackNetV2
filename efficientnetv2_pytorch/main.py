...
import torch
from datasets import TrackNetV2Dataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default='s', choices=['s', 'm', 'l'], help='EfficientNetV2 model type')
    args = parser.parse_args()

    # 修改這裡，使用絕對路徑
    train_dataset = TrackNetV2Dataset(csv_path='./Dataset/labels_train.csv')
    ...
    val_dataset = TrackNetV2Dataset(csv_path='./Dataset/labels_val.csv')
    ...
    # 修改這裡的模型
    # 根據參數選擇 EfficientNetV2 模型
    if args.model_type == 's':
        backbone = efficientnetv2_s(in_channels=9)
    elif args.model_type == 'm':
        backbone = efficientnetv2_m()
        # 若要支援 in_channels=9，需修改 efficientnetv2_m 定義
    elif args.model_type == 'l':
        backbone = efficientnetv2_l()
        # 若要支援 in_channels=9，需修改 efficientnetv2_l 定義

    # 若 TrackNetV2 需自訂 backbone，可這樣寫
    from efficientnetv2_pytorch.tracknetv2 import TrackNetV2
    model = TrackNetV2(backbone=backbone)
    device = 'cuda'
